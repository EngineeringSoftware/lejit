...
public class Sha2Cryptm292TemplateGEN1 {
    ...
    public static String sha256Crypt(final byte[] keyBytes, String salt) {
        if (salt == null) {
            salt = SHA256_PREFIX + B64.getRandomSalt(-1172028779);
        }
    ...
    public static String sha256Crypt(final byte[] keyBytes, String salt, final Random random) {
        if (salt == null) {
            salt = SHA256_PREFIX + B64.getRandomSalt(intVal().eval(2), random);
        }
        return sha2Crypt(keyBytes, salt, SHA256_PREFIX, SHA256_BLOCKSIZE, MessageDigestAlgorithms.SHA_256);
    }
    ...
    private static String sha2Crypt(final byte[] keyBytes, final String salt, final String saltPrefix, final int blocksize, final String algorithm) {
        ...
        if (m.group(intVal().eval(6)) != null) {
            rounds = Integer.parseInt(m.group(3));
            rounds = Math.max(intId().eval(7), Math.min(ROUNDS_MAX, rounds));
            roundsCustom = boolVal().eval(8);
        }
        final String saltString = m.group(intVal().eval(9));
        ...
        while (relation(intId(), intId()).eval(13) && variableNameThatWillNotExist471146962 < 1000) {
        ...
        while (relation(intId(), intVal()).eval(18) && variableNameThatWillNotExist1725074079 < 1000) {
        ...
        while (relation(intId(), arithmetic(intId(), intId())).eval(22) && variableNameThatWillNotExist1503131635 < 1000) {
        ...
        while (relation(intId(), arithmetic(intId(), intId())).eval(29) && variableNameThatWillNotExist13544240 < 1000) {
    ...
    public static String sha512Crypt(final byte[] keyBytes, String salt) {
        if (salt == null) {
            salt = SHA512_PREFIX + B64.getRandomSalt(intVal().eval(137));
        }
    ...
    public static String sha512Crypt(final byte[] keyBytes, String salt, final Random random) {
        if (salt == null) {
            salt = SHA512_PREFIX + B64.getRandomSalt(intVal().eval(138), random);
        }
        return sha2Crypt(keyBytes, salt, SHA512_PREFIX, SHA512_BLOCKSIZE, MessageDigestAlgorithms.SHA_512);
    }
    ...
    public static long main0(String[] args) {
        ...
        cs.updateStaticFieldsOfClass(Sha2Cryptm292TemplateGEN1.class);
        return cs.getValue();
    }
