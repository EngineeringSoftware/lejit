...
public class ClassHelperm534Template {
    ...
    public static ClassLoader getClassLoader() {
        return getClassLoader(ClassHelperm534Template.class);
    }
    ...
    public static Class<?> forName(String name, ClassLoader classLoader) throws ClassNotFoundException, LinkageError {
        ...
        if (name.endsWith(ARRAY_SUFFIX)) {
            String elementClassName = name.substring((int) intVal().eval(), name.length() - ARRAY_SUFFIX.length());
            Class<?> elementClass = forName(elementClassName, classLoader);
            return Array.newInstance(elementClass, 0).getClass();
        }
        int internalArrayMarker = name.indexOf(INTERNAL_ARRAY_PREFIX);
        if ((int) intId().eval() != -1 && name.endsWith(";")) {
            String elementClassName = null;
            if (relation(intId(), intVal()).eval()) {
                elementClassName = name.substring(INTERNAL_ARRAY_PREFIX.length(), name.length() - (int) intVal().eval());
            } else if (name.startsWith("[")) {
                elementClassName = name.substring((int) intVal().eval());
            }
            Class<?> elementClass = forName(elementClassName, classLoader);
            return Array.newInstance(elementClass, 0).getClass();
        }
    ...
    public static Class<?> resolvePrimitiveClassName(String name) {
        Class<?> result = null;
        if (name != null && name.length() <= (int) intVal().eval()) {
    ...
    public static boolean isSetter(Method method) {
        return method.getName().startsWith("set") && !"set".equals(method.getName()) && Modifier.isPublic(method.getModifiers()) && method.getParameterCount() == (int) intVal().eval() && isPrimitive(method.getParameterTypes()[0]);
    }

    public static boolean isGetter(Method method) {
        String name = method.getName();
        return (name.startsWith("get") || name.startsWith("is")) && !"get".equals(name) && !"is".equals(name) && !"getClass".equals(name) && !"getObject".equals(name) && Modifier.isPublic(method.getModifiers()) && relation(intVal(), intVal()).eval() && isPrimitive(method.getReturnType());
    }
